...
test('shared image captcha component renders the server challenge and keeps its id', () => {
  ...
  const types = read(
    'admin/packages/effects/common-ui/src/components/captcha/types.ts',
  );

  assert.match(
    component,
    /defineModel<string>/,
    'captcha answer is a form model',
  );
  assert.match(
    types,
  ...
  assert.match(
    types,
    /onChallengeId\??:/,
    'component exposes the challenge id seam',
  );
  assert.match(
    component,
    /props\.request/,
    'component invokes the challenge request',
  );
  assert.match(component, /payload/, 'server image payload is rendered');
  assert.match(
    component,
    /expiresIn/,
    'challenge expiry is retained for the UI',
  );
  assert.match(
    component,
    /data-testid=["']image-captcha["']/,
    'stable UI test target',
  );
  assert.match(
    component,
    /type=["']button["']/,
    'refresh control is keyboard actionable',
  );
});
...
test('all management login forms consume image challenges and submit captchaId', () => {
  for (const ui of managementUIs) {
    const login = read(
      `admin/apps/${ui}/src/views/_core/authentication/login.vue`,
    );
    ...
    assert.match(
      login,
      /rules:\s*z\.string\(\)\.optional\(\)/,
      `${ui} keeps the default-off risk policy server-driven`,
    );
    assert.match(
      login,
      /captchaId:\s*captchaId\.value/,
      `${ui} submit challenge id`,
    );
    assert.doesNotMatch(
      login,
      /SliderCaptcha/,
      `${ui} does not use the deferred slider provider`,
    );
    assert.match(
      store,
