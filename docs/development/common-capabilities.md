...
5. SMTP、媒体库页面在 `web-antd`、`web-ele`、`web-naive` 保持路由、字段、权限、文案和交互语义等价；邮件服务拆为 SMTP 账户、调用者、通知模板、验证码策略、投递记录五个 Tag，并提供“普通使用流程”“开发者对接流程”“调用示例”引导抽屉。
...
3. **调用示例：** 可复制的模板测试、验证码挑战/校验和投递记录查询请求；路径、变量和响应字段与公开 API 文档保持一致。

当前三套 UI 共用轻量 guide schema（标题、受众、普通步骤、开发者步骤、示例和 locale 文案），前端本地化静态 JSON 优先；抽屉支持 Esc/遮罩关闭、再次打开及焦点返回。步骤 ID、代码块、权限、错误 key、外链、已读状态和租户重置属于后续 P5 扩展。后台编辑需求进入独立 guide 资源与权限设计，静态引导继续与邮件模板分离。
...
- 引导：普通/开发者/调用示例三个分段、关闭后重开、多语言缺译回退、错误 key 与示例代码可复制。
