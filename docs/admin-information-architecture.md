...
> 适用版本：`1.0.0-dev`。本文是生产侧栏、路由、权限与页面内容的权威清单，同时记录当前运行基线与已确认的下一版目标；各节明确标注状态，历史 checkpoint 中的旧菜单只代表当时状态。
...
1. 三套 UI 登录后只显示五组目标菜单，顺序、文案、访问码和空态一致。
2. 旧深链可访问并确定性收敛到 canonical，服务端仍执行最终授权。
3. fixture/live、scope、degraded/unavailable/stale/zero 语义在页面和接口一致。
4. P7 证据包含 OpenAPI、migration、视觉/a11y、patch、manifest、verification record 和 rollback；在此之前第 2 节表格继续作为运行基线。
