...
async function saveTask() {
  ...
  notice.value = '';
  try {
    form.payloadSchema = JSON.parse(form.payloadText) as Record<
      string,
      unknown
    >;
  } catch {
...
async function cancelRun(item: TaskRun) {
  ...
  if (
    !selected.value ||
    !window.confirm(String($t('page.tasks.cancelRunConfirm')))
  )
    return;
...
async function retryRun(item: TaskRun) {
  ...
  if (
    !selected.value ||
    !window.confirm(String($t('page.tasks.retryRunConfirm')))
  )
    return;
  runAction.value = item.id;
  error.value = '';
...
<template>
  <ManagementPage
    class="tasks-page"
    :aria-busy="loading || saving || Boolean(running) || Boolean(runAction)"
    ...
    <section class="workspace-grid">
      <article class="panel" aria-labelledby="tasks-list-title">
        <div class="section-heading">
          <div>
            <p class="eyebrow">{{ $t('page.tasks.listEyebrow') }}</p>
            <h2 id="tasks-list-title">{{ $t('page.tasks.listTitle') }}</h2>
          </div>
        </div>
      ...
      <article class="panel" aria-labelledby="tasks-form-title">
        <div class="section-heading">
          <div>
            <p v-if="canManage" class="eyebrow">
              {{ $t('page.tasks.formEyebrow') }}
            </p>
            <h2 id="tasks-form-title">
              {{
                canManage
                  ? editingId
                    ? $t('page.tasks.editTitle')
                    : $t('page.tasks.newTitle')
                  : $t('page.tasks.runsTitle')
              }}
            </h2>
          </div>
        </div>
        ...
        <section v-if="selected" class="runs" aria-labelledby="task-runs-title">
          <div class="section-heading">
            <h3 id="task-runs-title">{{ $t('page.tasks.runsTitle') }}</h3>
            <button
              class="secondary"
              type="button"
              @click="loadRuns(selected.id)"
            >
              {{ $t('page.tasks.refreshRuns') }}
            </button>
          </div>
          <p v-if="runs.length === 0" class="empty-state">
            {{ $t('page.tasks.runsEmpty') }}
          </p>
          <ul v-else>
