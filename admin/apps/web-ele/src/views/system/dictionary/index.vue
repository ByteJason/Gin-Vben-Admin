...
const selectedType = computed(() =>
  types.value.find((item) => item.code === selectedCode.value),
);
const canEditType = computed(() =>
  Boolean(selectedType.value && !selectedType.value.systemOwned),
);
...
async function loadTypes() {
  ...
  error.value = '';
  try {
    types.value = await listDictionariesApi({
      includeDisabled: includeDisabled.value,
    });
...
async function removeType(type: DictionaryType) {
  ...
  if (
    type.systemOwned ||
    !window.confirm(String($t('page.dictionary.confirmTypeDelete')))
  )
    return;
...
async function removeItem(item: DictionaryItem) {
  ...
  if (
    item.systemOwned ||
    !window.confirm(String($t('page.dictionary.confirmItemDelete')))
  )
    return;
...
async function importItems() {
  ...
  error.value = '';
  try {
    await importDictionaryItemsApi(
      selectedCode.value,
...
<template>
  <ManagementPage
    ...
    :aria-busy="loading || saving || importing"
      ...
      <article class="panel" aria-labelledby="dictionary-types-title">
        <div class="section-heading">
          <div>
            ...
            <h2 id="dictionary-types-title">
              {{ $t('page.dictionary.typesTitle') }}
            </h2>
          </div>
          <button
            v-if="canManage"
        ...
        <div class="table-scroll">
          <table>
            <caption class="sr-only">
              {{
                $t('page.dictionary.typesTableLabel')
              }}
            </caption>
            <thead>
              <tr>
                <th scope="col">{{ $t('page.dictionary.code') }}</th>
                <th scope="col">{{ $t('page.dictionary.status') }}</th>
                <th scope="col">{{ $t('page.dictionary.sortOrder') }}</th>
                <th scope="col">{{ $t('page.dictionary.actions') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="!loading && types.length === 0">
                <td class="table-state" colspan="4">
                  {{ $t('page.dictionary.emptyTypes') }}
                </td>
              </tr>
              <tr
                v-for="type in types"
                :key="type.id"
                :class="{ selected: selectedCode === type.code }"
              >
                <th scope="row">
                  <button
                    class="link-button"
                ...
                <td class="actions">
                  ...
                  ><button
                    ...
                    class="danger"
                    type="button"
                    :disabled="deleting === type.code"
                    @click="removeType(type)"
                  >
                    {{ $t('page.dictionary.delete') }}
                  </button>
                </td>
              </tr>
            </tbody>
        ...
        <template v-else>
          ...
          <div class="table-scroll">
            <table>
              <caption class="sr-only">
                {{
                  $t('page.dictionary.itemsTableLabel')
                }}
              </caption>
              <thead>
                <tr>
                  <th scope="col">{{ $t('page.dictionary.value') }}</th>
                  <th scope="col">
                    {{ $t('page.dictionary.localizedLabel') }}
                  </th>
                  <th scope="col">{{ $t('page.dictionary.status') }}</th>
                  <th scope="col">{{ $t('page.dictionary.sortOrder') }}</th>
                  <th scope="col">{{ $t('page.dictionary.actions') }}</th>
          ...
          <details v-if="canManage" class="import-card">
            ...
            >
              {{
                importing
                  ? $t('page.dictionary.importing')
                  : $t('page.dictionary.import')
              }}
            </button>
          </details>
        </template>
...
.workspace-grid {
  display: grid;
  grid-template-columns: minmax(360px, 0.85fr) minmax(480px, 1.5fr);
  gap: 24px;
  margin-top: 24px;
}
...
.panel {
  padding: 24px;
  background: color-mix(in srgb, #fff 94%, #dbeafe);
