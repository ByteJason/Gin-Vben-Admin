...
export function testNotificationTemplateApi(id: string, input?: { locale?: string; recipient?: string; variables?: Record<string, unknown> }) { return requestClient.post<{ isTest: boolean; messageId: string; status: string; }>(notificationPath(ADMIN_ENDPOINTS.testNotificationTemplate, id), input); }
