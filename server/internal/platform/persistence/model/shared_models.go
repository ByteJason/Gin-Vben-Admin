...
type AppMetadata struct {
	...
	Version       uint64     `gorm:"column:version;not null;check:chk_app_metadata_version,version > 0;comment:版本号"`
...
type Tenant struct {
	...
	Name      string     `gorm:"column:name;size:191;not null;uniqueIndex:uq_tenants_name;comment:租户名称"`
	Status    string     `gorm:"column:status;size:32;not null;default:active;check:chk_tenants_status,status IN ('active','disabled');comment:租户状态"`
	CreatedAt time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:创建时间"`
	UpdatedAt time.Time  `gorm:"column:updated_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:更新时间"`
	DeletedAt *time.Time `gorm:"column:deleted_at;precision:6;comment:删除时间"`
}
...
type Organization struct {
	...
	TenantID  string     `gorm:"column:tenant_id;size:64;not null;uniqueIndex:uq_organizations_tenant_name,priority:1;index:idx_organizations_tenant_parent,priority:1;comment:租户标识"`
	ParentID  *string    `gorm:"column:parent_id;size:64;index:idx_organizations_tenant_parent,priority:2;comment:上级组织"`
	Name      string     `gorm:"column:name;size:191;not null;uniqueIndex:uq_organizations_tenant_name,priority:2;comment:组织名称"`
	Status    string     `gorm:"column:status;size:32;not null;default:active;check:chk_organizations_status,status IN ('active','disabled');comment:组织状态"`
	CreatedAt time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:创建时间"`
	UpdatedAt time.Time  `gorm:"column:updated_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:更新时间"`
	DeletedAt *time.Time `gorm:"column:deleted_at;precision:6;comment:删除时间"`
}
