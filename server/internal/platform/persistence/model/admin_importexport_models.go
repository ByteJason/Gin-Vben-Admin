...
type ImportExportJob struct {
	...
	Kind           string     `gorm:"column:kind;size:16;not null;index:idx_import_export_scope_status,priority:3;check:chk_import_export_kind,kind IN ('import','export');comment:作业类型"`
	TenantID       string     `gorm:"column:tenant_id;size:128;not null;uniqueIndex:uq_import_export_scope_idempotency,priority:1;index:idx_import_export_scope_status,priority:1;comment:租户标识"`
	OrgID          string     `gorm:"column:org_id;size:128;not null;default:'';uniqueIndex:uq_import_export_scope_idempotency,priority:2;index:idx_import_export_scope_status,priority:2;comment:组织标识"`
	...
	QueueTaskID    string     `gorm:"column:queue_task_id;size:64;not null;default:'';index:idx_import_export_queue_task;comment:队列任务标识"`
	IdempotencyKey string     `gorm:"column:idempotency_key;size:191;not null;uniqueIndex:uq_import_export_scope_idempotency,priority:3;comment:幂等键"`
	Status         string     `gorm:"column:status;size:32;not null;default:pending;index:idx_import_export_scope_status,priority:4;check:chk_import_export_status,status IN ('pending','running','succeeded','failed','cancelled');comment:作业状态"`
	...
	ProcessedRows  int32      `gorm:"column:processed_rows;not null;default:0;check:chk_import_export_progress,processed_rows <= total_rows;comment:已处理行数"`
	...
	CreatedAt      time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);index:idx_import_export_scope_status,priority:5;comment:创建时间"`
...
type ImportExportError struct {
	...
	JobID      string     `gorm:"column:job_id;size:64;not null;index:idx_import_export_errors_job_row,priority:1;comment:作业标识"`
	RowNumber  int32      `gorm:"column:row_number;not null;index:idx_import_export_errors_job_row,priority:2;comment:行号"`
	...
	DeletedAt  *time.Time `gorm:"column:deleted_at;precision:6;comment:删除时间"`
	CreatedAt  time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);index:idx_import_export_errors_job_row,priority:3;comment:创建时间"`
	UpdatedAt  time.Time  `gorm:"column:updated_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:更新时间"`
}

func (ImportExportError) TableName() string { return "import_export_errors" }

type ImportExportArtifact struct {
	ID        string     `gorm:"column:id;size:64;primaryKey;comment:工件标识"`
	JobID     string     `gorm:"column:job_id;size:64;not null;uniqueIndex:uq_import_export_artifact_job;comment:作业标识"`
	...
	ExpiresAt *time.Time `gorm:"column:expires_at;precision:6;index:idx_import_export_artifact_expiry;comment:过期时间"`
	DeletedAt *time.Time `gorm:"column:deleted_at;precision:6;comment:删除时间"`
	CreatedAt time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:创建时间"`
	UpdatedAt time.Time  `gorm:"column:updated_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);comment:更新时间"`
}

func (ImportExportArtifact) TableName() string { return "import_export_artifacts" }
