...
type TaskDefinition struct {
	...
	Enabled           bool       `gorm:"column:enabled;not null;default:true;index:idx_gvba_task_definitions_scope_enabled,priority:3;comment:是否启用"`
	Concurrency       int32      `gorm:"column:concurrency;not null;default:1;check:chk_gvba_task_definitions_concurrency,concurrency > 0;comment:并发数"`
	ConcurrencyPolicy string     `gorm:"column:concurrency_policy;size:16;not null;default:forbid;check:chk_gvba_task_definitions_policy,concurrency_policy IN ('allow','forbid','replace');comment:并发策略"`
	TimeoutMS         int64      `gorm:"column:timeout_ms;not null;default:30000;comment:超时毫秒数"`
	MaxAttempts       int32      `gorm:"column:max_attempts;not null;default:1;check:chk_gvba_task_definitions_attempts,max_attempts > 0;comment:最大尝试次数"`
