...
type AuthAuditEvent struct {
	...
	TenantID  string     `gorm:"column:tenant_id;size:64;not null;default:default;index:idx_gvba_audit_auth_events_tenant,priority:1;comment:租户标识"`
	OrgID     *string    `gorm:"column:org_id;size:64;index:idx_gvba_audit_auth_events_tenant,priority:2;comment:组织标识"`
	UserID    *uint64    `gorm:"column:user_id;index:idx_gvba_audit_auth_events_user_created,priority:1;comment:用户标识"`
	SessionID string     `gorm:"column:session_id;size:64;not null;default:'';comment:会话标识"`
	EventType string     `gorm:"column:event_type;size:64;not null;index:idx_gvba_audit_auth_events_type_created,priority:1;comment:事件类型"`
	Category  string     `gorm:"column:category;size:16;not null;default:operation;index:idx_gvba_audit_auth_events_category_created,priority:1;comment:事件分类"`
	...
	CreatedAt time.Time  `gorm:"column:created_at;precision:6;not null;default:CURRENT_TIMESTAMP(6);index:idx_gvba_audit_auth_events_user_created,priority:2;index:idx_gvba_audit_auth_events_type_created,priority:2;index:idx_gvba_audit_auth_events_category_created,priority:2;comment:创建时间"`
...
func (AuthAuditEvent) TableName() string { return "gvba_audit_auth_events" }
